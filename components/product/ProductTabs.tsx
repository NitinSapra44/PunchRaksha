...
export function ProductTabs({ product, className = "" }: { product?: any, className?: string }) {
  ...
  const getTabLabel = (tabKey: string) =>
    tabKey === "Product Info" ? (product?.productDetails?.tabTitle || "Product Details") : tabKey;
    ...
    switch (activeTab) {
      ...
      case "Product Info":
        return (
          <div className="animate-fadeIn">
            <h2 className="font-outfit txt-h2 !font-semibold text-[#111] mb-[15px] md:mb-[30px]">
              {(productDetails as any).productLabel || `About ${product?.name || "PunchRaksha"}`}
            </h2>
          ...
          {visibleTabs.map((tab) => (
            <button
              ...
              className={`px-[15px] py-[0px] md:px-[15px] md:py-2.5
                max-md:!font-[600] btn-radius-5 md:font-medium tracking-[0.033em] transition-all h-[50px] md:h-[55px] flex-shrink-0 whitespace-nowrap border ${activeTab === tab

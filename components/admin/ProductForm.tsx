...
import { uploadResizedImageToR2 } from "@/lib/utils/uploadImage";
...
export function ProductForm({ initialData = {}, isEdit = false }: Props) {
  ...
  const handleOgImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = ""; // allow re-selecting the same file
    try {
      const url = await uploadResizedImageToR2(file, 1200);
      setOgImageUrl(url);
    } catch (err) {
      toast.error((err instanceof Error ? err.message : "Image upload failed"));
    }
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <div className="flex gap-2 mb-2">
                    <input type="file" accept="image/*" onChange={handleOgImageUpload} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#045830] file:text-white hover:file:bg-[#034620]" />
                  </div>
                ...
                {featuredImageMode === "product" ? (
                  ...
                  <label className="flex flex-col items-center justify-center gap-1 border-2 border-dashed border-[#045830]/40 rounded-lg p-3 cursor-pointer hover:bg-[#045830]/5 transition">
                    ...
                    <input type="file" accept="image/*" className="hidden" onChange={async e => {
                      ...
                      e.target.value = ""; // allow re-selecting the same file
                      try {
                        setFeaturedImage(await uploadResizedImageToR2(file, 800));
                      } catch (err) {
                        toast.error((err instanceof Error ? err.message : "Image upload failed"));
                      }
                    }} />
              ...
              <div className="flex-1 space-y-2">
                <label className="flex flex-col items-center justify-center gap-1 border-2 border-dashed border-[#045830]/40 rounded-lg p-3 cursor-pointer hover:bg-[#045830]/5 transition">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={async e => {
                    ...
                    e.target.value = ""; // allow re-selecting the same file
                    try {
                      setConsultationImage(await uploadResizedImageToR2(file, 800));
                    } catch (err) {
                      toast.error((err instanceof Error ? err.message : "Image upload failed"));
                    }
                  }} />
